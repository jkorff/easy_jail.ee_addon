...
*
* This version changed by Jens Korff, Creative Spirits, to include iframe jailing
*
	...
	function _loadImage ( options, $img ) {
		...
		if ($img[0].nodeName == "IMG") {
			...
			cache.onload = function() {
				// Prevent duplicate concurrent calls for the same image object
				if ( undefined === $img.attr('data-src') ) return;

				cache_onload(cache, $img, options);
			};
		...
		// Video (iframe) caching
		else if ($img[0].nodeName == "IFRAME") {
			// Check if iframe has already been loaded
			if ( $('iframe[src="' + $img.attr("data-src") + '"]').length > 0 ) {
				return;
			}
			...
			// Wait for iframe to be ready for DOM manipulation
			// http://stackoverflow.com/questions/1463581/wait-for-iframe-to-load-in-javascript
			$(frame).load( function(){
				cache_onload(frame, $img, options);
			})

			frame.src = $img.attr("data-src");
		}
	}

	// Common cache function for images and iframes
	function cache_onload(cache, $img, options) {
		...
		$(cache).remove(); // Required for iframes
